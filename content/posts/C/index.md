...
tags = [ 'tideman', 'bitwise operation', 'graph theory', 'adjacency list', 'adjacency matrix', 'C', 'summary']
series_order = 2
+++

{{< lead >}}
The adjacency matrix is a square matrix used to represent a finite graph in graph theory. However, it can be space-consuming for sparse graphs. This article incorporates bitwise operations into the application of the adjacency matrix in the Tideman voting algorithm and achieves O(n) for finding the winner. Importantly, it also condenses the processes of locking pairs and finding winners into a concise 18 lines of code while optimising memory usage by {{< katex >}}\\(\(\frac{n-1}{n}\)\\).
{{< /lead >}}
...
**Tally**
Suppose that there are 3 candidates {a, b, c} and 10 voters, such that the votes are tallied as shown in [Table 1](#table1).
<a name="table1"></a>
<p align="center">
...
**Sort**
The pairs are then sorted in decreasing order by strength of victory.
<a name="table2"></a>
<p align="center">
...
**Lock**
In this step, the Tideman algorithm locks the pairs from the strongest strength and avoids creating cycles. To represent this, we can create a matrix with all the initial values set as "0", and then use "1" to represent "locked" pairs. 
<a name="table3"></a>
<p align="center">
...
The graph representing the above process is depicted in [Figure 1](candidate-graph.png). According to the Tideman method, the victor of the election is the candidate who never appears as a "loser"; consequently, they should be the “source” of the graph with no arrows pointing towards them. Therefore, in this case, candidate C is considered the winner.
...
### 2.1. Record Votes
The `vote` function collects ballots from voters and ensures their validity by verifying if the names appear in the `candidate` list. After validation, the function proceeds to record the votes.
...
2. Iterate through all the pairs in `pairs`:
   - Check if the new pair will create a cycle with existing connections in `NEW`. If not:
       ...
       - For each newly added pair, update all the connections in `NEW`. To build the connection, simply check if the winner of the new pair ever appeared as the loser in the previous pair, and update the new connection by adding it to the list.

{{< alert "bell" >}}
When creating new connections (links), ensure to avoid duplication to prevent exceeding the size of the array.
{{< /alert >}}
...
In the context of tideman, pair `{pair[i].winner,pair[i].loser}` will represent as winner<sup>th</sup> bit in the binary format of `NEW[pair[i].loser]`. We can use `1 << n` to represent n<sup>th</sup> item of `NEW[i]` (e.g.: `1 << 2` = `100`, which sets the 2<sup>th</sup> bit to 1). We can use `&` to check if n<sup>th</sup> item exist and use `|` to set n<sup>th</sup> item to 1.
...
1. Create a new empty array `NEW` with the size of the maximum candidate count and initialise all elements of the array to zero
2. Iterate through all the pairs in `pairs`:
	- Use `&` to check if the new pair will create a cycle with existing connections in `NEW` by verifying if the loser<sup>th</sup> item in the winner<sup>th</sup> column already exists. If not:
         - Lock the new pair in `locked` and add this pair in `NEW` using `|=`.
```c
void lock_pairs(void)
{
    int NEW[MAX] = {};
...
The `print_winner` function will print out the name of the candidate who is the winner under Tideman algorithm. The winner should be the "source" of the graph meaning that no arrows point to him/her. 

For the adjacency list and adjacency matrix method, the `print_winner` checks every `loser` column, and breaks if meets any `true`.
```c
...
#### 2.5.1. Using Bitwise Operation
For the bitwise operation method, `print_winner` just needs to simply find the `0`s in the array.
```c
void print_winner(void)
{
...
Big thanks to Juju for making the exploration journey so fun and CS50 staff for bringing us such an amazing [exercise](https://cs50.harvard.edu/x/2020/psets/3/tideman/)!
